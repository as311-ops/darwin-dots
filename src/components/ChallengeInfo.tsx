...
export default function ChallengeInfo({ challenge }: ChallengeInfoProps) {
  ...
  return (
    <div className="bg-zinc-900 rounded-lg border border-zinc-800 px-3 py-2 flex items-center gap-2">
      <span className="bg-emerald-700 text-white text-[9px] font-bold px-1.5 py-0.5 rounded flex-shrink-0">
        CHALLENGE
      </span>
      <span className="text-xs font-medium text-zinc-200 flex-shrink-0">{info.title}</span>
      <span className="text-xs text-zinc-500 truncate hidden sm:block">{info.flavor}</span>
    </div>
  );
}
